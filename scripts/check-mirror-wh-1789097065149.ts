...
async function main() {
  ...
  const checkoutId = 'wh-1789097065149';

  const { data: contacts, error: cErr } = await db
    .from('contacts')
    .select('id, name, phone, created_at, account_id')
    .or('phone.like.%9513668779%,phone.eq.919513668779,phone.eq.9513668779');
  if (cErr) throw cErr;
  ...
  const { data: pending, error: pErr } = await db
    ...
    .select(
      'id, checkout_id, contact_id, status, run_at, error_message, created_at, updated_at, payload',
    )
