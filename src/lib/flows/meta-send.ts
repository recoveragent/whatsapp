...
interface SendAddressEngineArgs {
  accountId: string
  userId: string
  conversationId: string
  contactId: string
...
/**
 * Send a WhatsApp Address Message from the Flows engine and persist
 * the outbound prompt as an interactive bot message.
 */
export async function engineSendAddressMessage(
  args: SendAddressEngineArgs,
): Promise<{ whatsapp_message_id: string }> {
  const db = supabaseAdmin()
