...
const ACTIONS: Action[] = [
  ...
  { labelKey: 'newDeal', href: '/pipelines', icon: Briefcase, tint: 'text-secondary' },
  { labelKey: 'newBroadcast', href: '/broadcasts/new', icon: Radio, tint: 'text-amber-600' },
...
export function QuickActions() {
  ...
  const actions = ACTIONS.filter(
    (a) => a.href !== '/pipelines' || isLeadGenBrand,
  )
      ...
      {actions.map((a) => {
        ...
        return (
          <Link
            key={a.href}
            href={a.href}
            className="group flex items-center gap-3 rounded-lg border border-border bg-card px-4 py-3 transition-all duration-200 hover:border-border-lit"
          >
