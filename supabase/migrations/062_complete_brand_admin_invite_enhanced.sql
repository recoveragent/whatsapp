-- ============================================================
-- 062_complete_brand_admin_invite_enhanced.sql
--
-- Super admins can manually assign a brand admin when email
-- confirmation or /join redeem failed. Matches redeem_invitation
-- orphan-account cleanup; also finds auth.users by invited email
-- when no profile row exists yet. Expired invites can still be
-- completed manually.
-- ============================================================
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    ...
    INTO v_user_id
    FROM auth.users u
    WHERE lower(trim(u.email)) = lower(trim(v_inv.invited_email))
    LIMIT 1;
  END IF;
